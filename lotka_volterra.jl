...
res3 = DiffEqFlux.sciml_train(loss, res1.minimizer, BFGS(initial_stepnorm=0.01f0), cb=callback, maxiters = 10000)
println("Final training loss after $(length(losses)) iterations: $(losses[end])")

res4 = DiffEqFlux.sciml_train(loss, res3.minimizer, BFGS(initial_stepnorm=0.01f0), cb=callback, maxiters = 10000)
println("Final training loss after $(length(losses)) iterations: $(losses[end])")

losses

pl_losses = plot(1:101, losses[1:101], yaxis = :log10, xaxis = :log10, xlabel = "Итерации", ylabel = "Потери", label = "ADAM (Shooting)", color = :blue)
plot!(102:144, losses[102:144], yaxis = :log10, xaxis = :log10, xlabel = "Итерации", ylabel = "Потери", label = "BFGS (Shooting)", color = :red)
plot!(145:length(losses), losses[145:end], color = :black, label = "BFGS (L2)")
...
pl_trajectory = scatter(t, transpose(Xₙ), color = :black, label = ["Измерения" nothing], xlabel = "t", ylabel = "x(t), y(t)")
plot!(tsample, transpose(X̂), color = :red, label = ["Предсказания UDE" nothing])
savefig("repos\\UDE_proj\\pres\\ude_predict_lotka.png")
...
savefig("repos\\UDE_proj\\pres\\ude_missing_term_predict_lotka.png")
...
function loss_fit(θ)
    X̂ = solve(estimation_prob, Tsit5(), p = θ, saveat = t)
    sum(abs2, X̂ .- Xₙ)
end
